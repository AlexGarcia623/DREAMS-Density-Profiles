...
### Get Densities ###
sat_params = []
bh_mass    = np.load('./data/stellar_mass.npy')

slopes = np.zeros((len(boxes),output_size))

for box in boxes:
    slopes[box,0] = bh_mass[box]
    slopes[box,1] = np.log10(sim_params[box,4] * 0.1 * (10**bh_mass[box])*2e30 * (3e8**2))
...
def objective(trial):
    ...
    for i in range(hparams.n_layers):
        of.append(trial.suggest_int("n_units_l{}".format(i), 4, 1000))
