...
## note that I am intentionally not dividing by h below
##   I saved the data differently for dmo and hydro sims
rvir_dmo   = np.load('./data/rvir_dmo.npy') 

with h5py.File(in_file, 'r') as file:
    for index, box in enumerate(np.arange(0,1024)):
        if box in [846,796,10,15,448,476,503,656,704,410,547,581,781,808,829,984,745]:
            slopes[box] = np.nan
...
sat_params = sat_params[no_nans]
slopes     = slopes[no_nans]

input_data = em.normalize_data(slopes, log=False)
#####################
